...
mindmap
  root((Dynamic Programming<br/>Patterns))
    Linear_DP(📈 Linear DP)
      Step_Pattern["Step Pattern<br/>(Fibonacci Style)"]
      Take_Skip["Take or Skip<br/>(House Robber Style)"]
      String_Part["String Partitioning<br/>(Decoding Style)"]
      State_Mach_Lin["State Machine<br/>(Stock Trading)"]
    Interval_DP(↔️ Interval DP)
      Merge_Cut["Merging / Cutting<br/><br/>(Matrix Chain Style)"]
      Palin_Exp["Palindrome Expansion"]
      Game_Ends["Game Theory<br/>(Ends Selection)"]
    String_DP(🔤 String DP)
      LCS["Longest Common<br/>Subsequence (LCS)"]
      Edit_Dist["Edit Distance /<br/>Transformation"]
      Str_Match["String Matching<br/>(Wildcards)"]
    Bitmask_DP(🎭 Bitmask DP)
      TSP["Ordering & Routing<br/>(TSP Style)"]
      Part_Group["Partitioning & Grouping<br/>(Knapsack Style)"]
      Game_Win["Game Theory<br/>(Winning State)"]
      Grid_Prof["Grid Profile<br/>(Broken Profile)"]
    Grid_DP(🕸️ Grid DP)
      Path_Count["Path Counting<br/>(Unique Paths)"]
      Path_Sum["Min/Max Path Sum<br/>(Accumulation)"]
      Obstacles["Grid with Obstacles"]
      Back_DP["Directional Future<br/>Dependency (Backward)"]
    State_Machine(🚦 State Machine)
      Binary["Binary States<br/>(Hold vs. Empty)"]
      Cooldown["States with Cooldown"]
      K_Trans["Count-Constrained<br/>(k-Transactions)"]
      Prop_Track["Property Tracking<br/>(Sequence Validity)"]
    Tree_DP(🌲 DP on Trees)
      Sel_Skip["Select or Skip<br/>(House Robber on Tree)"]
      Path_Prob["Path Problems<br/>(Diameter / Max Path)"]
      Subtree["Subtree Queries<br/>(Info Passing)"]
    Knapsack_DP(🎒 Knapsack DP)
      0_1_Knap["0/1 Knapsack<br/>(Use Max Once)"]
      Unbound["Unbounded Knapsack<br/>(Use Infinitely)"]
      Multi_Const["Multi-Constraint<br/>(2D Costs)"]
      Target_Sum["Target Sum /<br/>Plus-Minus Variation"]
    Digit_DP(🔢 Digit DP)
      Prop_Count["Property Counting<br/>(Restricted Structure)"]
      Digit_Sum["Digit Sum / Remainder"]
      Spec_Set["Specific Digit Set<br/>(Construction)"]
    Multi_Dim(🧊 Multi-Dimensional)
      Grid_Steps["Grid Traversal<br/>with Steps (3D)"]
      Part_Arr["Partitioning Arrays<br/>(K groups)"]
      Gap_Dim["Sequence with Property<br/>(Gap Dimension)"]
...
### 11. DP + Alpha (Tricks/Data Structures)
