...
**Concept:**
The GCD of two integers is the largest number that divides both of them without leaving a remainder. The most efficient way to calculate it is the **Euclidean Algorithm**.

**Logic:** $GCD(a, b) = GCD(b, a \pmod b)$ until $b$ becomes 0.

[Image of Euclidean algorithm diagram]
...
**Concept:**
The LCM is the smallest positive integer that is divisible by both numbers. It is directly related to the GCD by the formula:
$$LCM(a, b) = \frac{|a \times b|}{GCD(a, b)}$$

**Example: LCM(4, 6)**

1.  $GCD(4, 6) = 2$.
2.  Product = $4 \times 6 = 24$.
3.  $LCM = 24 / 2 = 12$.
...
public class LCM {
    ...
    public static int lcm(int a, int b) {
        if (a == 0 || b == 0) return 0;
        return (a * b) / gcd(a, b); 
        // Note: For large numbers, divide before multiplying to avoid overflow:
        // return (a / gcd(a, b)) * b;
    }
...
You have covered a very strong core of Number Theory. However, strictly for **LeetCode and Technical Interviews**, there are **three** major concepts missing from your list that appear frequently.

Here are the missing concepts, formatted exactly like your notes.

-----

### 6\. Euclidean Algorithm (GCD & LCM)

**Concept:**
The Greatest Common Divisor (GCD) is the largest number that divides two integers without a remainder. The Least Common Multiple (LCM) is the smallest number that is a multiple of both.
This is arguably **the most common** math concept in coding interviews (used in array rotations, string patterns, and flow problems).

**Formula:**

  - **GCD:** `gcd(a, b) = gcd(b, a % b)` (Recursively reduce until b is 0).
  - **LCM:** `(a * b) / gcd(a, b)` (Be careful of overflow: compute `(a / gcd) * b`).
...
public class GCD_LCM {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("LCM of 4, 6: " + lcm(4, 6));     // 12
    }
}
```

### Matrix Exponentiation (Fast Fibonacci / Linear Recurrences)

**Concept:**
You already know "Modular Exponentiation" to compute $x^n$. Matrix Exponentiation uses the same logic to compute the $n$-th term of a linear recurrence (like Fibonacci) in $O(\log n)$ time instead of $O(n)$.

If you represent the Fibonacci transition as a matrix:
$$\begin{bmatrix} F_{n+1} \\ F_n \end{bmatrix} = \begin{bmatrix} 1 & 1 \\ 1 & 0 \end{bmatrix} \begin{bmatrix} F_n \\ F_{n-1} \end{bmatrix}$$

To find the $n$-th term, you raise the matrix to power $n-1$:
$$\begin{bmatrix} 1 & 1 \\ 1 & 0 \end{bmatrix}^{n-1}$$

**Why use it?**
If LeetCode asks for the "billionth Fibonacci number" (Climbing Stairs with $n=10^9$), standard DP ($O(n)$) will Time Limit Exceeded (TLE). This method works in milliseconds.
...
public class MatrixExponentiation {
    ...
    // Main function to get Nth Fibonacci
    public static long getNthFibonacci(int n) {
        ...
        // The result is in the top-left cell (F_n) derived from F_1 and F_0
        return result[0][0];
    }
    ...
    public static void main(String[] args) {
        // Calculates 100th Fibonacci number instantly
        System.out.println("Fib(100): " + getNthFibonacci(100)); 
    }
}
```

### Reservoir Sampling (Randomization)

**Concept:**
How do you select $k$ elements from a stream of data of **unknown size** $n$ such that every element has an equal probability of being chosen? You cannot store all elements (memory constraint).

**Algorithm (Algorithm R):**

1.  Fill the "reservoir" (array) with the first $k$ elements.
2.  For the $i$-th element (where $i > k$):
      * Generate a random index $j$ between $0$ and $i$.
      * If $j < k$, replace `reservoir[j]` with the new element.

**Why use it?**
Classic interview question: "Pick a random line from a huge file that doesn't fit in RAM." (LeetCode 382, 398).
...
public class ReservoirSampling {
    ...
    public static int[] selectKItems(int[] stream, int k) {
        ...
        // Step 1: Fill reservoir with first k elements
        for (int i = 0; i < k; i++) {
        ...
        for (int i = k; i < stream.length; i++) {
            ...
            // If the random index falls inside the reservoir, replace it
            if (j < k) {
    ...
    public static void main(String[] args) {
        ...
        int[] sample = selectKItems(stream, 3);
        
        System.out.print("Random Sample: ");
        for (int num : sample) System.out.print(num + " ");
    }
}
```
